...
class PasswordResetRequestController extends Controller
{
    ...
    public function sendEmail(Request $request)  // this is most important function to send mail and inside of that there are another function
    {
        ...
        $this->send($request->email, $user[0]->name);  //this is a function to send mail 
        return $this->successResponse();
    }

    public function send($email, $name)  //this is a function to send mail 
    {
        $token = $this->createToken($email);
        Mail::to($email)->send(new SendMailReset($token, $email, $name));  // token is important in send mail 
    }
